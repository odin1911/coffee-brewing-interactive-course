...
- 文档澄清：README 将考题要求的前端服务作为主运行路径，并把可选日志服务拆为独立章节，记录未启动时的静默降级和重连方式。
...
| 持续记录与本地 API 复验 | recorder/store/rendering 定向测试；真实 API 连续追加 4 类事件 | 测试通过，单文件持续更新且无临时文件残留 | 14/14 通过；会话累计 1250ms，保存答题、图表事件和 completed 状态 | 通过 |
| 工具页复验 | React 渲染断言与本地 `/tools` HTTP 请求 | JSON 文案存在，路由可访问 | rendering 8/8 通过，`/tools` 返回 200；本次浏览器连接工具初始化失败，未重复视觉验收 | 通过（非视觉） |
| 一键 PDF 导出 | `npm run export:pdf` | 无手动操作生成并保留交付文件 | `exports/course.pdf`，9 页、1152×648pt、中文和图片正常 | 通过 |
...
| 2026-07-14 | 浏览器连接工具初始化持续报告 `Cannot redefine property: process` | 2 | 不改项目或插件；使用 React 渲染断言和真实 HTTP 检查完成非视觉复验，并明确保留视觉验收限制 |

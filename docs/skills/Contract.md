...
- Images accept local paths, HTTPS URLs, or base64 raster data URLs. HTTP, executable protocols, and non-image data URLs are rejected.
...
- Chart data is native button/CSS UI with a top-level `details` entry for every drill-down item.

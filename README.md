...
| 完整打印版 | 同一份完整配置 | `/course` 与 `/print` 复用同一画布和页面组件，不维护第二份内容或版式 |
...
5. `/print` 用共享页面组件遍历同一份 `slides[]`，保留全部答题分支，并在对应图表主页面后生成明细续页。
...
- `/print`：全部页面定义、分支和图表明细续页；当前咖啡课程为 9 个主页面加 2 个明细续页。确认所有图片加载后再调用浏览器打印，生成的 PDF 已包含图片，离线打开不需要联网。
